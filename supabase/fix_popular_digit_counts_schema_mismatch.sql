-- Fix production schema mismatch causing:
-- "column window_key is of type smallint but expression is of type text" (42804)
--
-- This recreates popular-digit tables/functions with correct column types for AM/PM windows.

BEGIN;

-- Drop dependent functions/triggers first.
DROP TRIGGER IF EXISTS bets_after_insert_popular ON public.bets;
DROP FUNCTION IF EXISTS public.trg_bets_popular_digit();
DROP FUNCTION IF EXISTS public.get_trending_digits();
DROP FUNCTION IF EXISTS public.get_popular_digits(TEXT);
DROP FUNCTION IF EXISTS public.popular_digit_record_first_pick(UUID, INT, TIMESTAMPTZ);
DROP FUNCTION IF EXISTS public.popular_digit_ensure_counts_for_window(TEXT);

-- Recreate tables with correct schema.
DROP TABLE IF EXISTS public.popular_digit_counts;
DROP TABLE IF EXISTS public.popular_digit_first_pick;

CREATE TABLE public.popular_digit_first_pick (
...
CREATE INDEX IF NOT EXISTS idx_popular_first_window
  ON public.popular_digit_first_pick (window_key);

CREATE TABLE public.popular_digit_counts (
  window_key TEXT NOT NULL,
  digit SMALLINT NOT NULL CHECK (digit >= 0 AND digit <= 99),
...
CREATE INDEX IF NOT EXISTS idx_popular_counts_window
  ON public.popular_digit_counts (window_key);
...
CREATE POLICY "popular_digit_first_pick none"
  ON public.popular_digit_first_pick FOR ALL USING (false);

DROP POLICY IF EXISTS "popular_digit_counts none" ON public.popular_digit_counts;
CREATE POLICY "popular_digit_counts none"
  ON public.popular_digit_counts FOR ALL USING (false);

-- Functions (AM/PM window logic comes from popular_digit_window_key()).
