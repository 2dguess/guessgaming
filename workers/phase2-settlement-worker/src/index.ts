...
async function claimPendingRows(client: PoolClient, limit: number): Promise<ClaimedRow[]> {
  const query = `
    WITH picked AS (
      SELECT q.queue_id
  ...
  `;
  const res = await client.query<ClaimedRow>(query, [limit]);
  return res.rows;
}

async function applyBatchTransaction(client: PoolClient, rows: ClaimedRow[]): Promise<{ appliedRows: number; adminDelta: number }> {
  ...
  try {
    ...
    const inserted = await client.query<{ amount: string }>(
      `
      WITH payload AS (
        SELECT
...
async function moveBatchToDlq(client: PoolClient, rows: ClaimedRow[], errMessage: string): Promise<void> {
  const queueIds = rows.map((r) => r.queue_id);
  const betIds = rows.map((r) => r.bet_id);
...
async function processOneTick(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `[phase2-worker] applied rows=${result.appliedRows}, admin_delta=${result.adminDelta}, claimed=${claimed.length}`
      );
    } catch (err) {
...
async function publishSettlementApplied(
  result: { appliedRows: number; adminDelta: number },
  claimedRows: number
): Promise<void> {
  if (!supabase) return;
...
async function bootstrap(): Promise<void> {
  ...
  console.log(
    `[phase2-worker] started queue=${QUEUE_NAME} every=${SCHEDULE_EVERY_MS}ms batchSize=${BATCH_SIZE}`
  );
}
